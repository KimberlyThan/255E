...
# Read each excel worksheet as individual 
for (i in 1:length(sheetList)){assign(paste0("df", i), as.data.frame(sheetList[i]))}

# data = rbind.data.frame(df1, df2, df3)
```
...
# Average values in r1 - r6 (internal control, baseline) of each row 
df1$avgBL = rowMeans(df1[, 11:16])
df2$avgBL = rowMeans(df2[, 11:16])
df3$avgBL = rowMeans(df3[, 11:16])

# Normalize all values by avgBL as a percentage of baseline 
df1$percentBL = df1[ ,11:28]/df1$avgBL*100
df1$percentBL = df1[ ,11:28]/df1$avgBL*100
df1$percentBL = df1[ ,11:28]/df1$avgBL*100

# Average FST at each time point of all TC trials 
avgTCFST = aggregate(df1[ ,-c(1:29)], by = list(df1$TimeControl.Position), mean)


sdTCFST = aggregate(df1[ ,-c(1:29)], by = list(df1$TimeControl.Position), sd)
nTCFST = aggregate(df1[ ,-c(1:29)], by = list(df1$TimeControl.Position), length)

SETCFST=sdTCFST[,-1]/sqrt(nTCFST[,-1])
lowerSE = avgTCFST[,-1] - SETCFST
upperSE = avgTCFST[,-1] + SETCFST


AnNum =unique(df1$TimeControl.AnimalNum)

for (i in 1:length(AnNum)){
  tempDP = df1[df1$TimeControl.AnimalNum == AnNum[i] , ]
  
  DP = tempDP[tempDP$TimeControl.Position== 'DP', 11:28]
  IST = tempDP[tempDP$TimeControl.Position=='IST',11:28]
  difference = DP-IST
  
  if (i == 1){
      df1Diff = difference
    df1Diff$AnimalNumber = AnNum[i]
  } else {
     difference$AnimalNumber = AnNum[i]
     df1Diff = rbind(df1Diff, difference)
  }
